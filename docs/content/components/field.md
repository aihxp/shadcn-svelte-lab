...
- **Vertical fields:** Default orientation stacks label, control, and helper text, which is ideal for mobile-first layouts.
...
The `Field.Legend` has two variants: `legend` and `label`. The `label` variant applies label sizing and alignment. This is useful when you have a nested `Field.Set`.
